...
import { ICON_QR_CODE, MAIN_COLOR, ICON_ARROW_DROP, ICON_CHECKED, BLACK } from '../../constants';
import API from '../../api';
...
const unit1 = ['Chọn'];
const unit2 = ['Chọn'];
import { QR_SCAN_CODE_INVENTORY, } from '../../redux/types'
...
class ModalDrugDQG extends PureComponent<Props, State> {
  ...
  componentWillReceiveProps(nextProps) {
    const info = nextProps.navigation.state.params && nextProps.navigation.state.params.info && nextProps.item.drug_id === nextProps.navigation.state.params.info.drug_id
  ...
  onPressOK = () => {
    ...
    if (lot && expired_date && parent_unit_price && import_unit_price && import_qty && selectedValueDosageForms && selectedValueGroupDrug && unit && source) {
    ...
    return (
      ...
      >
        <View style={styles.wapperInputFilter}>
          ...
          <Image
            source={ICON_ARROW_DROP}
            style={{ width: 7, height: 7, tintColor: 'black', position: 'absolute', right: 5 }}
            resizeMode="contain"
          />
        </View>
      </ModalDropdown>
  ...
  selectValueUnit1 = (idx, value) => {
    value === 'Trống' ? this.setState({ selectValueUnit1: '', unit_name1: '', unit_price1: '', unit_qty1: '' }) :
      this.setState({ selectValueUnit1: value, unit_name1: this.props.unitType[idx].code });
  };
  selectValueUnit2 = (idx, value) => {
    value === 'Trống' ? this.setState({ selectValueUnit2: '', unit_name2: '', unit_price2: '', unit_qty2: '' }) :
  ...
  scanQRCode = async () => {
    ...
    navigation.navigate(QR_SCAN_CODE_INVENTORY, { item: this.props.item, info: info });
  };
      ...
      >
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
          <View style={styles.buttonCheck}>
            {!status && <Image source={ICON_CHECKED} style={styles.iconCheck} />}
          </View>
        ...
        <View style={[styles.containerBody, { alignItems: 'center', paddingHorizontal: 5, justifyContent: 'center' }]}>
          ...
          <View style={[styles.containerLayout, { marginTop: 5 }]}>
            <View style={styles.rowLeft}>
              ...
              {lot && lot.length !== 0 ? null : (
                <Text style={{ position: 'absolute', right: 10, marginTop: 0, color: 'red' }}>*</Text>
              )}
            </View>
          ...
          <View style={[styles.containerLayout, { marginTop: 5, justifyContent: 'space-between' }]}>
            <View style={{ flexDirection: 'row', flex: 3, alignItems: 'center' }}>
              ...
              {expired_date && expired_date.length !== 0 ? null : (
                <Text style={{ position: 'absolute', right: 10, marginTop: 0, color: 'red' }}>*</Text>
              )}
            </View>
            <View style={{ flexDirection: 'row', flex: 7, alignItems: 'center' }}>
              ...
              <View style={{ flexDirection: 'row', flex: 6, alignItems: 'center' }}>
                <View>
                  <Text style={{ paddingLeft: 5 }}>Nhóm thuốc  </Text>
                  {selectedValueGroupDrug && selectedValueGroupDrug.length !== 0 ? null : (
                    <Text style={{ position: 'absolute', right: 0, marginTop: 0, color: 'red' }}>*</Text>
                  )}
                </View>
                <View style={{ flex: 1 }}>
          ...
          <View style={[styles.containerLayout, { marginTop: 5, justifyContent: 'space-between' }]}>
            <View style={{ flexDirection: 'row', flex: 3, alignItems: 'center' }}>
              <Text>Số lượng</Text>
              {import_qty && import_qty.length !== 0 ? null : (
                <Text style={{ position: 'absolute', right: 10, marginTop: 0, color: 'red' }}>*</Text>
              )}
            </View>
            <View style={{ flexDirection: 'row', flex: 7, alignItems: 'center' }}>
              ...
              <View style={{ flexDirection: 'row', flex: 6, alignItems: 'center' }}>
                <View>
                  <Text style={{ paddingLeft: 5 }}>Đơn vị cơ bản</Text>
          ...
          <View style={[styles.containerLayout, { marginTop: 5 }]}>
            <View style={styles.rowLeft}>
              <Text>Giá nhập (ĐVCB)</Text>
              {import_unit_price && import_unit_price.length !== 0 ? null : (
                <Text style={{ position: 'absolute', right: 10, marginTop: 0, color: 'red' }}>*</Text>
              )}
            </View>
            <View style={styles.rowRight}>
          ...
          <View style={[styles.containerLayout, { marginTop: 5 }]}>
            <View style={styles.rowLeft}>
              <Text>Giá bán   (ĐVCB)</Text>
              {parent_unit_price && parent_unit_price.length !== 0 ? null : (
                <Text style={{ position: 'absolute', right: 10, marginTop: 0, color: 'red' }}>*</Text>
              )}
            </View>
            <View style={styles.rowRight}>
  ...
  _renderContentOption = () => {
    const { unit_price1, unit_price2, selectValueUnit1, unit_qty1, selectValueUnit2, unit_qty2, unit_name1, unit_name2 } = this.state
    const empty = {
      code: "000",
      type: "DRUG_UNIT",
      value: "Trống",
    ...
    return (
      <View style={[styles.containerBody, { alignItems: 'center', paddingHorizontal: 5, justifyContent: 'center', paddingBottom: 10 }]}>
        <View style={[styles.containerLayout, { marginTop: 5, justifyContent: 'space-between', alignItems: 'center' }]}>
          ...
          <View style={{ flexDirection: "row", flex: 5, alignItems: 'center', marginLeft: vw(5) }}>
            <Text style={styles.text}>Giá trị{'\n'}quy đổi 1
                        {unit_qty1 && unit_qty1.length !== 0 ? null : (
                <Text style={{ marginTop: 0, color: 'red' }}>*</Text>
              )}</Text>
            <View style={{ flex: 4 }}>
              {this._renderInfomation('unit_qty1', unit_name1.length !== 0 ? 'Nhập số lượng' : 'Chọn ĐVQĐ1', 'numeric', unit_name1.length !== 0 ? true : false, { textAlign: 'center' })}
            </View>
          </View>
        </View>
        <View style={[styles.containerLayout, { marginTop: 5 }]}>
          <Text style={[styles.text, { flex: 2 }]}>Giá bán ĐVQĐ1
                    {unit_price1 && unit_price1.length !== 0 ? null : (
              <Text style={{ marginTop: 0, color: 'red' }}>*</Text>
            )}</Text>
          <View style={{ flex: 8 }}>
            {this._renderInfomation('unit_price1', unit_name1.length !== 0 ? 'Nhập giá bán ĐVQĐ 1' : 'Xin vui lòng chọn ĐVQĐ1', 'numeric', true, unit_name1.length !== 0 ? true : false)}
          </View>
        </View>
        ...
        <View style={[styles.containerLayout, { marginTop: 5, justifyContent: 'space-between', alignItems: 'center' }]}>
          ...
          <View style={{ flexDirection: "row", flex: 5, alignItems: 'center', marginLeft: vw(5) }}>
            <Text style={styles.text}>Giá trị{'\n'}quy đổi 2
                        {unit_qty2 && unit_qty2.length !== 0 ? null : (
                <Text style={{ marginTop: 0, color: 'red' }}>*</Text>
              )}</Text>
            <View style={{ flex: 4 }}>
              {this._renderInfomation('unit_qty2', unit_name2.length !== 0 ? 'Nhập đơn vị' : 'Chọn ĐVQĐ2', 'numeric', unit_name2.length !== 0 ? true : false, { textAlign: 'center' })}
            </View>
          </View>
        </View>
        <View style={[styles.containerLayout, { marginTop: 5 }]}>
          <Text style={[styles.text, { flex: 2 }]}>Giá bán ĐVQĐ2
                    {unit_price2 && unit_price2.length !== 0 ? null : (
              <Text style={{ marginTop: 0, color: 'red' }}>*</Text>
            )}</Text>
          <View style={{ flex: 8 }}>
            {this._renderInfomation('unit_price2', unit_name2.length !== 0 ? 'Nhập giá bán ĐVQĐ 2' : 'Xin vui lòng chọn ĐVQĐ2', 'numeric', unit_name2.length !== 0 ? true : false)}
          </View>
        </View>
      </View>
    )
