...
class ModalDrugBusiness extends PureComponent<Props, State> {
  ...
  unit_name = (idx: number, value: string) => {
    this.priceBeforeTax = this.state.typeUnit[idx].price;
    this.setState({
      unit_name: value,
      unit_cd: this.state.typeUnit[idx].code,
      price: this.state.typeUnit[idx].price,
    },()=>this.setState({amount: (this.state.typeUnit[idx].price*this.state.quantity).toFixed(0)}));
  };
  selectValueTax = async (idx: number, value: string) => {
    ...
    if (value === '5%') {
      this.setState({ price: (this.priceBeforeTax * Number(0.95)).toFixed(2) },() => this.setState({ amount: (this.state.quantity *this.priceBeforeTax * Number(0.95)).toFixed(2) }));
    } else if (value === '10%') {
      this.setState({ price: (this.priceBeforeTax * Number(0.9)).toFixed(2) },() => this.setState({ amount: (this.state.quantity *this.priceBeforeTax * Number(0.9)).toFixed(2) }));
    } else {
      this.setState({ price: this.priceBeforeTax.toFixed(2) },() => this.setState({ amount: (this.state.quantity *this.priceBeforeTax).toFixed(2) }));
  ...
  inputQuanlity = txt => {
    ...
    console.log(txt);
    console.log('====================================');
    this.setState({ quantity: txt }, () => this.setState({ amount: this.state.quantity * this.state.price }));
  };
  amount = txt => {
    this.setState({ amount: txt }, () =>
      this.setState({
        price: ((this.state.amount.length !== 0 ? this.state.amount : 0) / (this.state.quantity.length!==0?this.state.quantity:1)).toFixed(0),
      }),
    );
  };
  inputPrice = txt => {
    this.setState({ price: txt }, () => this.setState({ amount: this.state.quantity * this.state.price }));
  };
  renderBody = () => {
    const { item } = this.props;
    const { lot, expired_date, typeUnit, quantity, unit_name, price, selectValueTax, amount } = this.state;
    ...
    return (
      <ScrollView>
          ...
          <View style={{ flexDirection: 'row', flex: 7, alignItems: 'center' }}>
            <View style={{ flex: 4 }}>
              <TextInput
                ...
                onChangeText={txt => this.inputQuanlity(txt)}
              />
            </View>
        ...
        <View style={[styles.containerLayout, { marginTop: 5 }]}>
          <View style={styles.rowLeft}>
            <Text style={styles.txt12}>Thành tiền</Text>
          </View>
          <View style={styles.rowRight}>
            <TextInput
              ...
              style={styles.txtInput}
              onChangeText={txt => this.amount(txt)}
            />
          </View>
        </View>
      </ScrollView>
