...
export default {
  ...
  getLogin(username, password) {
    const url = `/drugstore/user?login_id=${username}&password=${MD5(password)}`;
    //const url = `/drg-mobile-api/drugstore/user?login_id=${username}&password=${type === 1 ? password : MD5(password)}`;
    return client.get(url, { apiName: arguments.callee.name }).then(response => {
  ...
  register(params) {
    ...
    return client.post(url, params, { apiName: arguments.callee.name }).then(response => response);
  },
  ...
  getDashboard(drg_store_id, drg_store_id_ncc, order_by, per_page) {
    ...
    const urlList = `/medlink/orderpool/list?page=${1}&per_page=${per_page || limit}`;
    let params = null;
    ...
    return axios.all([
      client.get(urlDashboard, {
        apiName: "getDashboard", headers: {
          Authorization: token
        }
      }),
      client.post(urlList, params, {
        apiName: "getListOrder", headers: {
          Authorization: token
        }
      }),
